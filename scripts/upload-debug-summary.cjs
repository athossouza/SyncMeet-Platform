...
const localScriptPath = path.join(__dirname, 'debug-summary-session.cjs');
const remoteScriptPath = '/root/syncmeet/scripts/debug-summary-session.cjs';

const conn = new Client();
console.log('Connecting to debug summary...');
...
conn.on('ready', () => {
    ...
    conn.sftp((err, sftp) => {
        ...
        sftp.fastPut(localScriptPath, remoteScriptPath, (err) => {
            ...
            const runCmd = `docker exec syncmeet node scripts/debug-summary-session.cjs`;
