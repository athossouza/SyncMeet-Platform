...
const localScriptPath = path.join(__dirname, 'debug-today.cjs');
const remoteScriptPath = '/root/syncmeet/scripts/debug-today.cjs';

const conn = new Client();
console.log('Connecting to deploy debug script...');
...
conn.on('ready', () => {
    ...
    conn.sftp((err, sftp) => {
        ...
        sftp.fastPut(localScriptPath, remoteScriptPath, (err) => {
            ...
            const runCmd = `docker exec syncmeet node scripts/debug-today.cjs`;
